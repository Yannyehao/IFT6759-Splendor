"""
Evaluation script for PPO Score-based agent (v2 - FIXED).

Uses the same SplendorGymWrapper as training to ensure correct action mapping.
Supports evaluation against RandomAgent (opponent=None) and GreedyAgent.

Date: 2026-02-25
Version: 2.0 (Fixed evaluation using Gym wrapper)
"""
...
def evaluate_vs_opponent(model, opponent_agent, num_games: int, max_turns: int = 200,
    ...
    Uses the SAME wrapper interface as training to ensure correct action mapping.
    ...
    for game_idx in tqdm(range(num_games), desc=desc):
        # Create fresh wrapper with same config as training
    ...
    stats = {
        ...
        'raw_agent_scores': [int(s) for s in agent_scores],
        'raw_opponent_scores': [int(s) for s in opponent_scores],
        'raw_game_lengths': [int(l) for l in game_lengths]
    }
...
def print_results(label: str, stats: dict):
    ...
    if stats['agent_scores']['max'] >= 15:
        print(f"  ✅ Agent reached 15+ points in at least one game (legitimate win)")
    if stats['opponent_scores']['max'] >= 15:
        print(f"  ℹ️  Opponent reached 15+ points in at least one game")


def main():
    parser = argparse.ArgumentParser(description='Evaluate PPO agent (v2 - fixed with wrapper)')
    ...
    parser.add_argument('--output', type=str,
                       default='project/experiments/evaluation/ppo_score_based_eval_v2',
                       help='Output directory')
    args = parser.parse_args()
    
    print("=" * 80)
    print("PPO Score-Based Agent Evaluation v2 (FIXED)")
    print("=" * 80)
    print(f"Model: {args.model}")
    print(f"Games per opponent: {args.games}")
    print(f"Max turns: {args.max_turns}")
    print(f"Using Gym wrapper (same as training)")
    print()
    ...
    print("  Using opponent=None (same random policy as training)")
    print("-" * 60)
    
    stats_random = evaluate_vs_opponent(
        model, opponent_agent=None, num_games=args.games,
        max_turns=args.max_turns, desc="PPO vs Random"
    )
    ...
    print("  Using RandomAgent(distribution='uniform_on_types')")
    print("-" * 60)
    ...
    stats_random_agent = evaluate_vs_opponent(
        ...
        max_turns=args.max_turns, desc="PPO vs RandomAgent"
    )
    ...
    print("  Using GreedyAgentBoost(mode='value')")
    print("-" * 60)
    ...
    stats_greedy = evaluate_vs_opponent(
        ...
        max_turns=args.max_turns, desc="PPO vs Greedy"
    )
    ...
    results_file = output_dir / f"eval_v2_{timestamp}.json"
